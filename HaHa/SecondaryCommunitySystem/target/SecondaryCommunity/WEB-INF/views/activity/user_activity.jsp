...
<head>
    ...
    <script type="text/javascript">
        ...
        function showRemoveDlg(join_id){
            delActivityNO=join_id;
            $("#message").html('您确认要删除的编号为['+join_id+']的参加用户吗?');
            $("#removeActivityModal").modal('show');
        }

        function removeActivity(){
            document.forms[0].action="<c:url value="/user_act/joins"/>/"+delActivityNO;
        ...
        <div style="float: left;">
            ...
            <div style="width: 1500px">
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        <c:forEach var="join" items="${joinList}" varStatus="idx">
                            <tr>
                                <th>${idx.index+1}</th>
                                <td>
                                    <div>参加人员账号：${join.join_userno}</div>
                                    <div>参加人员名称：${join.join_username}</div>
                                </td>
                                <td>
                                    <div>${join.join_actno}</div>
                                </td>
                                <td>
                                    <div>${join.join_actname}</div>
                                </td>
                                <td>
                                    <div>活动标题：${join.join_acttitle}</div>
                                    <div>活动时间：${join.join_acttime}</div>
                                    <div>活动积分：${join.join_actnum}</div>
                                </td>
                                <td>
                                    <div><button class="btn btn-primary btn-sm">给予奖励</button></div>
                                    <div style="padding-top: 10px"><button class="btn btn-danger btn-sm" onclick="showRemoveDlg('${join.join_id}')">删除</button></div>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
