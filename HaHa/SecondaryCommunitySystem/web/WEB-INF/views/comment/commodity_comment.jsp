...
<head>
    ...
    <script type="text/javascript">
        ...
        function removeComment(){
            document.forms[0].action="<c:url value="/comment/commodity"/>/"+delCommentNo;
        ...
        <div style="float: left;">
            ...
            <div>
                ...
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                    <c:forEach var="commcommodity" items="${commcommodityList}" varStatus="idx">
                        <tr>
                            <th>${idx.index+1}</th>
                            <td>
                                <div>${commcommodity.user_no}</div>
                            </td>
                            <td>
                                <div>${commcommodity.user_name}</div>
                            </td>
                            <td>
                                <div>${commcommodity.commdity_id}</div>
                            </td>
                            <td>
                                <div>${commcommodity.comment_time}</div>
                            </td>
                            <td>
                                <div>${commcommodity.commodity_comment}</div>
                            </td>
                            <td>
                                <div><button class="btn btn-danger btn-sm" onclick="showRemoveDlg('${commcommodity.comment_id}')">删除</button></div>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
